...
int load_image_buffer(LoadParams *params, void *buf, size_t len,
  ...
  if (imageType == JPEG) {
    // shrink: int, fail_on: int (enum VipsFailOn), autorotate: bool
    code = vips_jpegload_buffer(buf, len, out, "fail_on", params->fail_on,
...
int set_jpegload_options(VipsOperation *operation, LoadParams *params) {
  ...
  MAYBE_SET_BOOL(operation, params->fail_on, "fail_on");
